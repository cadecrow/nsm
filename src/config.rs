...
impl Config {
    pub fn load() -> Result<Self> {
        let config_path = Path::new("nsm.config.json");

        // If config file exists, load it
        if config_path.exists() {
            let config_str =
                fs::read_to_string(config_path).context("Failed to read nsm.config.json")?;

            let config: Config =
                serde_json::from_str(&config_str).context("Failed to parse nsm.config.json")?;
        ...
        fs::write(config_path, config_str).context("Failed to write default nsm.config.json")?;
