...
pub fn scan_project(project_path: &Path) -> Result<Vec<RouteInfo>> {
    ...
    for entry in WalkDir::new(&app_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if is_page_file(path) {
            ...
            let rel_path = path.strip_prefix(project_path)?.to_string_lossy().to_string();
...
fn default_label_for_route(route: &str) -> String {
    ...
    if let Some(last) = segments.last() {
        // Handle dynamic routes
        if last.starts_with(':') {
            return format!("{} Detail", last[1..].to_string());
        }
        
        // Capitalize first letter
        let mut chars = last.chars();
        if let Some(first_char) = chars.next() {
            return first_char.to_uppercase().collect::<String>() + chars.as_str();
        }
